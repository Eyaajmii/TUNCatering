...
async function generateBonLivraisonPDF(bonLivraison) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Populate commandes if they're not already populated
            const bonWithCommandes = await BonLivraison.findById(bonLivraison._id).populate('commandes');
...
exports.createBonLivraison = async (req, res) => {
    try {
        ...
        // Recherche du vol par son numéro (numVol)
        ...
        // Création du bon de livraison avec le numéro de vol (String)
        const newBonLivraison = new BonLivraison({
            numeroBon,
            vol: volId.toString(), // Stocke le numéro de vol comme string
            commandes,
        });
...
exports.getBonByVolId = async (req, res) => {
    try {
        ...
        // Recherche directe par le numéro de vol stocké (string)
        const bonLivraison = await BonLivraison.findOne({ vol: volId })
            .populate("commandes personnelLivraison");
        ...
        // Optionnel: Récupérer les infos du vol séparément
        const vol = await Vol.findOne({ numVol: volId });
        ...
        res.status(200).json({ 
            ...
            data: {
                ...
                volInfo: vol // Ajoute les infos du vol à la réponse
            }
        });
    } catch (error) {
...
exports.getBonLivraisonById = async (req, res) => {
    try {
        const { id } = req.params;
        const bonLivraison = await BonLivraison.findById(id).populate("vol commandes personnelLivraison");

        if (!bonLivraison) {
            return res.status(404).json({ success: false, message: "Bon de livraison introuvable." });
        }
...
exports.updateStatutBonLivraison = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Statut mis à jour", data: bonLivraison });
    } catch (error) {
