...
class RecommandationController {
  ...
  static async HistoriqueCommandes(MatriculePn) {
    try {
      const cmds= await commande.find({ MatriculePn: MatriculePn }).populate("plats") .populate({path:"menu",populate:[{path:"PlatsEntree"},{path:"PlatsPrincipaux"},{path:"PlatsDessert"},{path:"Boissons"}]});
  ...
  static async getMenuJour() {
    try {
      ...
      const menu = await Menu.findOne({ DateAjout: {$gte:date}, Disponible :true}).populate("PlatsEntree PlatsPrincipaux PlatsDessert Boissons");
      if (!menu) {
  ...
  static async recommandationGenerale(){
    ...
    if(menu){
      const menuDispo = await Menu.find({ Disponible: true }).populate("PlatsEntree PlatsPrincipaux PlatsDessert Boissons")
